...
ShadowMsg is a browser-based secure messaging platform built for private, real-time conversations. Every message is encrypted on your device before it leaves. The server never sees your plaintext. When a room is gone, the messages are gone with it.
...
![Share link bar at the bottom of the Create Room page or inside the empty chat state, showing the full join URL like "https://yourchat.vercel.app/join/roomname" with a green Copy button on the right side](Assets/Share_Link.png)
...
![Active Rooms lobby page showing two or three room cards in a grid layout, each card displaying a room name in monospace font, a green "Open" or red "Full" badge, a member count like "1 / 2 connected", small green name badge chips showing the names of connected users, a thin fill bar, and a Join Room button at the bottom of the card](Assets/Lobby_Screen.png)
...
![Active chat conversation showing at least 4 or 5 messages — green bubbles on the right side labeled "you", dark grey bubbles on the left labeled with the other user's name, and a small centered system message pill in the middle of the conversation reading something like "alice joined"](Assets/Chat.png)
...
![Bottom section of the chat message area showing the three small green dots in a bouncing animation next to the text "alice is typing" in grey monospace font, positioned just above the chat input bar](Assets/Demo_Typing.png)
...
![Chat message after a reaction has been added, showing one or two reaction badges underneath the message bubble — for example "👍 1" with a green-tinted border indicating the current user has reacted, and "😂 2" in a normal style](Assets/Reaction_Image.png)
...
![Close-up of the chat navigation bar at the top of the screen, showing the room name like "#myroom" on the left side, and directly below it the connected users displayed inline — one name in bright green with "(you)" appended, and another name in grey, separated by a comma](Assets/Copy_Image.png)
...
![Chat navigation bar with the lock button in its locked state showing a 🔒 icon highlighted in amber/yellow, and the room name also showing a small lock icon next to it indicating the room is currently locked](Assets/Lock_Image.png)
...
![Chat navigation bar highlighting the sound toggle button showing the 🔊 speaker icon, positioned between the other nav controls on the right side](Assets/Mute_Image.png)
...
![The panic confirmation dialog overlaid on top of the blurred chat background, showing a dark modal with a red "⚠ DESTROY ROOM?" title, a description warning that all messages will be lost and the action cannot be undone, a grey "Cancel" button and a red "Destroy Room" button side by side at the bottom](Assets/Boom1.png)

![The chat at the moment of destruction, showing the final system message pill reading "💥 Room destroyed by creator" appearing at the bottom of the message list just before the redirect happens](Assets/Boom2.png)
...
![Close-up of the chat header showing connected user names, with a small red or grey "✕" button clearly visible directly next to a non-creator user's name, ready to be clicked by the room creator to force remove them](Assets/Force_Remove.png)

![The kicked user's chat view at the moment they are removed, showing a system message reading "You were removed from the room by the creator" appearing at the bottom of their chat before the redirect](Assets/Force_Remove2.png)
...
> 📸 *[Screenshot: the ShadowMsg login screen as seen on a mobile phone (portrait orientation), showing the complete page fitting correctly within the screen — the logo at the top, the display name input, and the Continue button all visible without any zooming, scrolling, or horizontal overflow]*
